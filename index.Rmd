...
# .black[... Packages makes it easy to test our code `r emoji::emoji("silly")`] 
...
## 02: Steps to create a package II

.pull-left[

The function sets up the following components:

- DESCRIPTION File: A “package manifest” used by R and CRAN.

- R Folder: For all R Files.

- man folder: Contains automatically generated .Rd documentation files, created by roxygen2. 
...
.Rbuildignore File: All files and folders that will be ignored when building the package.
...
- ´load_all()´ parses all R files located in the R/ directory: This means devtools reads the R scripts in that folder, processes the code, and converts it into internal R objects that can be used immediately in your session.

- It loads all exported functions into your session (@export).

- It compiles documentation if you’re using roxygen2

=> This acts like hot-reloading for R development, it instantly reloads your package code into your session without needing a reinstall.
...
```
# Add a package to the DESCRIPTION
...
- Next, run devtools::document() to generate the documentation files based on your roxygen2 comments. 
...
- This will create or update the .Rd files in the man/ directory, which are used by R to provide help pages for your functions.
...
- It verifies that all functions are properly documented, using both roxygen2 annotations and the contents of the man/ directory.
- It ensures that the namespace is consistent and that all exported/imported functions are correctly declared.
- It checks that the overall package structure follows best practices and conforms to R package standards.
...
- If your package includes vignettes, it attempts to build and render them to confirm that they compile successfully.
